...
#print(plane_projectionND(data_virginica[0],(1,1,1,4)))
...
data_frame = data_sheet.rename({"Species": "Class"}, axis='columns')
data_frame = data_frame[["SepalLengthCm","SepalWidthCm","PetalLengthCm","Class"]]
...
def stat_data(points):
    classificator = lambda x: classifierND(x,n,b)
    targets = points[-1]
    objects = points[:-1]
...
n,b = find_classification_hyperplane(*setosa)
stat_data(setosa)

n,b = find_classification_hyperplane(*versicolor)
stat_data(versicolor)

n,b = find_classification_hyperplane(*virginica)  
stat_data(virginica)
"""
"""
def stat_data(points):
    classificator = lambda xyz: classifier3D(xyz[0],xyz[1],xyz[2],nx,ny,nz,b)
    x,y,z,targets = points
    objects = list(zip(x,y,z))
    accuracy = calculate_accuracy(objects,targets,classificator)
    precision, recall = calculate_precision_recall(objects,targets,classificator)
    accuracies.append(accuracy)
    precisions.append (precision)
...
nx,ny,nz,b = find_classification_plane(*setosa)
plot_classes3D(ax,nx,ny,nz,b,"red",*setosa)
stat_data(setosa)

nx,ny,nz,b = find_classification_plane(*versicolor)
plot_classes3D(ax,nx,ny,nz,b,"blue",*versicolor)
stat_data(versicolor)

nx,ny,nz,b = find_classification_plane(*virginica)
plot_classes3D(ax,nx,ny,nz,b,"green",*virginica)  
stat_data(virginica)
